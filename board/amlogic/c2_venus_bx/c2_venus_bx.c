...
int board_late_init(void)
{
	...
	/* Read GPIOA_8 (pgood) value */
	/* disable gpio pull */
	ret = readl(PADCTRL_GPIOA_PULL_EN);
	writel(ret & (~(0x1 << 8)), PADCTRL_GPIOA_PULL_EN);

	/* pin mux to gpio pin */
	/* GPIOA_8 */
	ret = readl(PADCTRL_PIN_MUX_REGC);
	writel(ret & (~(0xF << 0)), PADCTRL_PIN_MUX_REGC);

	/* enable input */
	ret = readl(PADCTRL_GPIOA_OEN);
	writel(ret | (0x1 << 8), PADCTRL_GPIOA_OEN);

	/* read pgood value */
	ret = readl(PADCTRL_GPIOA_I);
	pgood = (ret >> 8) & 0x1;
	...
	while (err) {
		pr_crit("Stop booting kernel due to low ac voltage\n");
		mdelay(1000);
		pr_crit("Retrying...\n");

		/* pgood may change within 4 seconds after transition from AC power to battery */
		/* Read new pgood value before check power */
		ret = readl(PADCTRL_GPIOA_I);
		pgood = (ret >> 8) & 0x1;
		pr_info("pgood=%d\n", pgood);
	...
	pr_info("Init led device after emmc tuning\n");
	sys_led_init(WHITE);
	return 0;
}
