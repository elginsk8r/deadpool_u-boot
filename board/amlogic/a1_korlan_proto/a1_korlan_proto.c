...
int board_late_init(void)
{
	...
	/* There are 12 RGB LED's on the EVK.  They are connected to GPIOF_6
	   (red), GPIOF_7 (green), and GPIOF_8 (blue).	Set them to inputs with
	   pull-down resistors to turn off the lights. (On SoC reset, red and
	   blue are partially lit by pull-ups.) */
	/*set GPIOF_6 and GPIO_7 as gpio function*/
	writel(readl(PADCTRL_PIN_MUX_REG6) & (~(0xFF << 24)), PADCTRL_PIN_MUX_REG6);
	/*set GPIOF_8 as gpio function*/
	writel(readl(PADCTRL_PIN_MUX_REG7) & (~(0xF << 0)), PADCTRL_PIN_MUX_REG7);
	/*enable input*/
	writel(readl(PADCTRL_GPIOF_OEN) | (0x7 << 6), PADCTRL_GPIOF_OEN);
	/*set pull down resistors*/
	writel(readl(PADCTRL_GPIOF_PULL_UP) & (~(0x7 << 6)), PADCTRL_GPIOF_PULL_UP);
	/*enable pull-up/down*/
	writel(readl(PADCTRL_GPIOF_PULL_EN) | (0x7 << 6), PADCTRL_GPIOF_PULL_EN);
