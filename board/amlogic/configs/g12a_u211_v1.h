...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"reboot_mode_android=${reboot_mode_android} logo=${display_layer},loaded,${fb_addr} "\
	"fb_width=${fb_width} fb_height=${fb_height} display_bpp=${display_bpp} "\
	"outputmode=${outputmode} vout=${outputmode},${vout_init} panel_type=${panel_type} "\
        ...
        "cmdline_keys="\
            "if keyman init 0x1234; then "\
                "if keyman read usid ${loadaddr} str; then "\
                    ...
                    "setenv serial ${usid};"\
                "else "\
					"setenv bootargs ${bootargs} androidboot.serialno=u211${cpu_id};"\
					"setenv serial u211${cpu_id};"\
                "fi;"\
