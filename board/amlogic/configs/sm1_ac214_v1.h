...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"setenv bootargs ${initargs} hdr_policy=${hdr_policy} hdr_priority=${hdr_priority} "\
		...
		"lcd_ctrl=${lcd_ctrl} lcd_debug=${lcd_debug} hdmitx=${cecconfig},${colorattribute} "\
		"hdmimode=${hdmimode} hdmichecksum=${hdmichecksum} "\
		"dolby_vision_on=${dolby_vision_on} frac_rate_policy=${frac_rate_policy} "\
		"hdmi_read_edid=${hdmi_read_edid} cvbsmode=${cvbsmode} "\
		"osd_reverse=${osd_reverse} video_reverse=${video_reverse} "\
		"irq_check_en=${Irq_check_en} jtag=${jtag};"\
        ...
        "cmdline_keys="\
            "if keyman init 0x1234; then "\
                ...
                "else "\
					"setenv bootconfig ${bootconfig} "\
					"androidboot.serialno=ac214${cpu_id};"\
					"setenv serial ac214${cpu_id};"\
                "fi;"\
