...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"boot_flag=0\0"\
        ...
        "initargs="\
            "init=/init " CONFIG_KNL_LOG_LEVEL " console=ttyS0,115200 no_console_suspend earlycon=aml-uart,0xff803000 ramoops.pstore_en=1 ramoops.record_size=0x8000 ramoops.console_size=0x4000 "\
            "\0"\
        "upgrade_check="\
	...
	"setenv bootargs ${initargs} phy_idx=${phy_idx} hdr_policy=${hdr_policy} ${fs_type} "\
	"hdr_priority=${hdr_priority} otg_device=${otg_device} "\
	"reboot_mode_android=${reboot_mode_android} "\
	"logo=${display_layer},loaded,${fb_addr} fb_width=${fb_width} fb_height=${fb_height} "\
	"display_bpp=${display_bpp} outputmode=${outputmode} "\
	"vout2=${outputmode2}, vout=${outputmode},${vout_init} panel_type=${panel_type} "\
	"lcd_ctrl=${lcd_ctrl} lcd_debug=${lcd_debug} hdmitx=${cecconfig},${colorattribute} hdmimode=${hdmimode} "\
        ...
        "cmdline_keys="\
            "if keyman init 0x1234; then "\
                "if keyman read usid ${loadaddr} str; then "\
                    ...
                    "setenv serial ${usid};"\
                "else "\
					"setenv bootargs ${bootargs} androidboot.serialno=u202${cpu_id};"\
					"setenv serial u202${cpu_id};"\
                "fi;"\
