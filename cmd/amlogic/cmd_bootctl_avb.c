...
{
    pr_info("info->magic = %s\n", info->magic);
    pr_info("info->version_major = %d\n", info->version_major);
    pr_info("info->version_minor = %d\n", info->version_minor);
    pr_info("info->slots[0].priority = %d\n", info->slots[0].priority);
    pr_info("info->slots[0].tries_remaining = %d\n", info->slots[0].tries_remaining);
    pr_info("info->slots[0].successful_boot = %d\n", info->slots[0].successful_boot);
    pr_info("info->slots[1].priority = %d\n", info->slots[1].priority);
    pr_info("info->slots[1].tries_remaining = %d\n", info->slots[1].tries_remaining);
    pr_info("info->slots[1].successful_boot = %d\n", info->slots[1].successful_boot);

    pr_info("info->crc32 = %d\n", info->crc32);
}
    ...
    char * const argv[])
{
    ...
    unsigned int bl2_part_value = 0;

    bl2_part_value = readl(SYSCTRL_SEC_STATUS_REG6);
    if (((bl2_part_value >> 25) & 0x7) == 1)
        env_set("bl2_part", "boot0");
    else if (((bl2_part_value >> 25) & 0x7) == 2)
        env_set("bl2_part", "boot1");
    else
        env_set("bl2_part", "user");
