	...
	char *const argv[])
{
	...
	/* if hpd_st high, output mode will be saved after edid_parse / mode select */
	if (!hpd_st) {
	...
	char * const argv[])
{
	...
	bool current_mode_support = false;
	/* hdmi_mode may be null or "none".
	 * if it is null or "none", it means user not
	 * selected manually, and need to select the best
	 * mode by policy
	 */
	bool user_prefer_is_set = false;
	...
	printf("read hdmichecksum: %s, user_hdmimode: %s, colorattribute: %s\n",
