				   ...
				   u_char *buffer)
{
	...
	if ((offset & (mtd->writesize - 1)) != 0) {
		pr_err("Attempt to read non page-aligned data\n");
		*length = 0;
		if (actual)
			*actual = 0;
		return -EINVAL;
	...
	if (need_skip < 0) {
		pr_err("Attempt to read outside the flash area\n");
		*length = 0;
		return -EINVAL;
	}

	if (used_for_read > lim) {
		pr_err("Size of read exceeds partition or device limit\n");
		*length = 0;
		return -EFBIG;
	}
	...
	while (left_to_read > 0) {
		...
		if (rval && rval != -EUCLEAN) {
			pr_err("%s %d read from offset %llx failed %d\n",
				__func__, __LINE__, offset, rval);
			*length -= left_to_read;
			return rval;
		}
			     ...
			     u_char *buffer, int flags)
{
	...
	if (flags & WITH_YAFFS_OOB) {
		...
		if (*length % (mtd->writesize + mtd->oobsize)) {
			pr_err("Attempt to write incomplete page");
			pr_err(" in yaffs mode\n");
			return -EINVAL;
		}
	} else
#endif
	 ...
	 */
	if ((offset & (mtd->writesize - 1)) != 0) {
		pr_err("Attempt to write non page-aligned data\n");
		*length = 0;
		return -EINVAL;
	}
	...
	if (need_skip < 0) {
		pr_err("Attempt to write outside the flash area\n");
		*length = 0;
		return -EINVAL;
	}
					...
					struct part_info *part)
{
	...
	while (start++ < end)
		if (mtd_block_isbad(mtd, (start*mtd->erasesize)))
			cnt++;
...
static loff_t mtd_store_ltop(loff_t off)
{
	...
	if (!mtd_map.init_flag) {
		pr_err("%s %d error: no valid map handler\n",
			__func__, __LINE__);
		return (loff_t)-1;
	}
	if (logic_blk > mtd_map.valid_max) {
		pr_err("%s %d error: over valid address\n",
			__func__, __LINE__);
		return (loff_t)-1;
	}
...
void mtd_store_init_map(void)
{
	...
	if (mtd->erasesize)
		...
		pr_debug("%s %d can not calculate block cnt\n",
			__func__, __LINE__);

	pr_debug("%s %d block cnt: 0x%x\n",
	...
	mtd_map.valid_max = j;
	mtd_map.init_flag = 1;
	pr_debug("%s %d: skipped %d, bad block need re-map: %d\n",
...
static int mtd_store_get_offset(const char *partname, loff_t *retoff, loff_t off)
{
	...
	*retoff = 0;
	if (!partname)
		...
		if (ret) {
			pr_err("%s %d can not find part:%s\n",
		...
		pr_debug("%s %d no mtd partition, offset to device 0 address\n",
			__func__, __LINE__);
		offset = off;
	}
			  ...
			  loff_t off, size_t size, void *dest)
{
	...
	if (ret) {
		pr_err("%s %d mtd read err, ret %d\n",
			__func__, __LINE__, ret);
		return ret;
	}
	if (retlen < size) {
		ret = -EIO;
		pr_err("%s %d mtd read err, size(%ld), retlen(%ld)\n",
			   ...
			   loff_t off, size_t size, void *source)
{
	...
	if (ret) {
		pr_err("%s %d mtd write err, ret %d\n",
			__func__, __LINE__, ret);
		return ret;
	}
	if (retlen < size) {
		ret = -EIO;
		pr_err("%s %d mtd write err, size(%ld), retlen(%ld)\n",
			__func__, __LINE__, size, retlen);
		return ret;
	}
	return ret;
			   ...
			   loff_t off, size_t size, int scrub_flag)
{
	...
	pr_debug("erasing from 0x%llx, length 0x%lx\n",
		   offset, size);
...
#ifdef CONFIG_DISCRETE_BOOTLOADER
	cpu_id_t cpu_id = get_cpu_id();
	char *boot_entry_old[] = {"bl2", "tpl"};
	char *boot_entry_sc2[] = {"bl2", "bl2e", "bl2x", "ddrfip", "devfip"};
	char **boot_entry = boot_entry_old;
