...
#ifdef CONFIG_AML_GPIO_OPTION
void OffLed(void)
{
	char *hw_id_env;
	unsigned long  hw_id;

	hw_id_env = env_get("hw_id");
	if (hw_id_env)
		hw_id = simple_strtoul(hw_id_env, NULL, 16);

	switch (hw_id) {
		case 0x3: /*p1-HYNIX ddr*/
		case 0xb: /*p1-SAMSUNG ddr*/
			/*led control: output high, turn off led*/
			gpio_option("GPIOA_5", GPIOD_IS_OUT, true);
			gpio_option("GPIOF_11", GPIOD_IS_OUT, true);
			gpio_option("GPIOP_3", GPIOD_IS_OUT, true);
			break;
		default:
			/*
			 *0x4: p2-HYNIX ddr
			 *0xc: p2-SAMSUNG ddr
			 */
			pr_info("%s,korlan-p2/b1/b3/b4\n", __func__);
			gpio_option("GPIOP_9", GPIOD_IS_OUT, true);
			gpio_option("GPIOF_11", GPIOD_IS_OUT, true);
			gpio_option("GPIOP_12", GPIOD_IS_OUT, true);
			break;
	}
}
#endif
...
#ifdef CONFIG_AML_GPIO_OPTION
	/*G-korlan: turn off led before boot kernel*/
	OffLed();
#endif
