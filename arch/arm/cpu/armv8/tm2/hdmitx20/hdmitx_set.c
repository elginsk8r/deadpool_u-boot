...
/*
 * arch/arm/cpu/armv8/tm2/hdmitx20/hdmitx_set.c
 *
...
static int hdmitx_get_hpd_state(void)
{
	...
	st = !!(hd_read_reg(P_PREG_PAD_GPIO2_I) & (1 << 16));
	return st;
}

static void ddc_pinmux_init(void)
{
	hd_set_reg_bits(P_PAD_PULL_UP_EN_REG2, 0, 23, 2);    // Disable GPIOH_23/24 pull-up/down
	hd_set_reg_bits(P_PAD_PULL_UP_REG2, 0, 23, 2);
	hd_set_reg_bits(P_PREG_PAD_GPIO2_EN_N, 3, 23, 2);     // GPIOH_23/24 input
	hd_set_reg_bits(P_PERIPHS_PIN_MUX_9, 1, 28, 4);      // Mux DDC SDA/SCL
	hd_set_reg_bits(P_PERIPHS_PIN_MUX_6, 1, 16, 4);
}
...
static void hdmitx_set_phy(struct hdmitx_dev *hdev)
{
	...
	case HDMI_720x576i50_4x3:
	case HDMI_720x576i50_16x9:
		set_phy_by_mode(hdev, HDMI_PHYPARA_270M);
		break;
