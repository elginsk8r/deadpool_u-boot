...
static int parse_kern_dtb(unsigned char *kern_dtb, unsigned kern_dtb_size,
	...
	if (expected_kernel_size != kern_dtb_size) {
		pr_info("the expected kern-dtb size is: %u\n", expected_kernel_size);
		pr_info("the actual kern-dtb size is: %u\n", kern_dtb_size);
...
ulong android_image_get_comp(const struct andr_img_hdr *os_hdr)
{
	...
	src = begin;
	for (i = 0; i < ARRAY_SIZE(lz4_magic); i++) {
		if (*src++ != lz4_magic[i])
			break;
	}
	if (i == ARRAY_SIZE(lz4_magic))
		return IH_COMP_LZ4;
