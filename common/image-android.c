...
// The kernel load address needs to be a multiple of 2MiB.
#define ANDROIDR_IMAGE_KERNEL_DECOMPRESS_LOAD_ADDR	0x01800000
...
static int android_image_need_move_v3(ulong *img_addr, const boot_img_hdr_v3_t *hdr)
{
	...
	*/
	...
	// For Kernel 5.15, the decompressed kernel size is larger than 32MiB.
	// We need to increate the margin to 64MiB.
	if (val < 64*1024*1024) {
